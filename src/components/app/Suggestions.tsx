...
// `Date.now()` is "impure" per react-hooks/purity, but Suggestions is an
// async Server Component that runs once per request — there is no render
// to re-execute. Date access at request time is exactly the right
// behaviour. Encapsulating the call here keeps the rule happy.
...
export async function Suggestions({ userId }: { userId: string }) {
  ...
  const [{ data: drifting }, { data: newThisWeek }, { data: pinnedQuiet }] =
    await Promise.all([
      supabase
        .from("contacts")
  ...
  const totalCards = driftingRows.length + newRows.length + pinnedRows.length;
  if (totalCards === 0) {
    return null; // hide entirely if nothing useful to suggest
  }
          ...
          {rows.map((r) => (
            ...
            >
              ...
              <Button asChild variant="ghost" size="sm" className="h-7 w-7 p-0">
                <Link
                  href={`/app/contact/${r.id}`}
                  aria-label={`Open ${r.display_name ?? r.email}`}
                >
